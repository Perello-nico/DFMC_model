...
def compute_DFMC_vectorize(moisture: float,
                           rain: float, temp: float, hum: float, wspeed: float,
                           saturation: float = SAT, T0: float = T0,
                           dt: float = DT,
                           params: Dict = DFMC_PARAMS
    ...
    """
    ## Dead Fuel Moisture Content (DFMC) [%] - FOR ARRAYS
    ...
    ### Return:
        ...
        4. (float): drying/wetting time (h)
    """
    moisture_new = np.empty_like(temp)
    phase = np.empty_like(temp)
    emc = np.empty_like(temp)
    K_const = np.empty_like(temp)
    # checks
    idx_no_data = check_no_data(rain=rain, temp=temp, hum=hum, wspeed=wspeed,
                                T0=T0, saturation=saturation)
    idx_no_update = check_no_update(temp=temp)
    idx_rain = check_rain(rain=rain, params=params)
    # no data - kept moisture for worst-case scenario
    moisture_new[idx_no_data] = moisture[idx_no_data]
    phase[idx_no_data] = NODATAVAL
    emc[idx_no_data] = NODATAVAL
    K_const[idx_no_data] = NODATAVAL
    # no update
    moisture_new[idx_no_update] = moisture[idx_no_update]
    phase[idx_no_update] = 0
    emc[idx_no_update] = NODATAVAL
    K_const[idx_no_update] = NODATAVAL

    # rain phase
    idx_rain_phase = idx_rain & (~idx_no_data) & (~idx_no_update)
    moisture_new[idx_rain_phase] = rain_phase(
        rain=rain[idx_rain_phase],
        moisture=moisture[idx_rain_phase],
        saturation=saturation[idx_rain_phase],
        params=params)
    phase[idx_rain_phase] = 2
    # no EMC and K
    emc[idx_rain_phase] = NODATAVAL
    K_const[idx_rain_phase] = NODATAVAL

    # no-rain phase
    moisture_new[~idx_rain_phase], phase[~idx_rain_phase], \
        emc[~idx_rain_phase], K_const[~idx_rain_phase] = no_rain_phase(
            moisture=moisture[~idx_rain_phase],
            temp=temp[~idx_rain_phase],
            wspeed=wspeed[~idx_rain_phase],
            hum=hum[~idx_rain_phase],
            saturation=saturation[~idx_rain_phase],
            T0=T0[~idx_rain_phase],
            dt=dt,
            params=params)
    return moisture_new, phase, emc, K_const
